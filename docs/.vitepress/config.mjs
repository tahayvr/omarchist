...
export default defineConfig({
  ...
  head: [
    ...
    [
      "meta",
      { name: "twitter:image", content: "/images/omarchist-social.png" },
    ],
  ],
  themeConfig: {
  ...
  transformHead({ assets }) {
    ...
    const jetBrainsFont = assets.find((file) =>
      /JetBrainsMono.*\.(woff2|woff|ttf)/.test(file),
    );
    if (jetBrainsFont) {
    ...
    const openSansFont = assets.find((file) =>
      /OpenSans.*\.(woff2|woff|ttf)/.test(file),
    );
    if (openSansFont) {
