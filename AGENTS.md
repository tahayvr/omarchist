...
- `runner.rs` runs steps one at a time: `Exec` through `setsid -f sh -c` (Omarchy's launch scripts `exec setsid`, which does not fork under `sh -c`, so a plain wait would block until the window closes) or `sh -c` when `wait` is set; `hyprctl dispatch` for Lua; `notify-send --`. It reports `RunEvent`s, honours `OnError`, and refuses nested loops via a call stack. A dispatcher of the form `omarchist flow run '<id>'` always becomes `StepKind::Flow` (`from_dispatcher`), so nesting is checked in process. `Runner::with_loader` takes a flow loader so tests never touch disk.
...
- Templates (`templates.rs`): `TemplateSource::{BuiltIn, User(path)}`; user templates are `~/.config/omarchist/templates/<slug>.flow.toml` with keys `user:<slug>`; `save_user_template` writes one (an export) but has no UI yet. A marketplace is a further source, never a new format. `requirements.rs` names the program of each command step and lists what is missing from `PATH`; the editor shows that as warnings, never as errors.
- Sharing (`share.rs`): `export_toml` strips the id and triggers; `read_import` reads a file or `https://` URL (64 KB cap, `meta.source` set for URLs) into an `Imported` flow with no id. Import never saves or runs: the UI opens the editor as `FlowEditSource::Imported` with a review banner, and the CLI prints the steps and asks unless `--yes`. Keep it that way; it is the safety gate for third-party flows.
