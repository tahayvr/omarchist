...
- Headless UI tests (`tests/keyboard_nav.rs`) open the real `Root`/`MainWindowView` with `gpui_kit::test`. They need the window free of network tasks (the Omarchy update watcher is started from `main.rs`) and background work on gpui's executor (`cx.background_spawn`, not `smol::unblock`) so the test scheduler can drive it.
