...
- Templates (`templates.rs`): `TemplateSource::{BuiltIn, User(path)}`; user templates are `~/.config/omarchist/templates/<slug>.flow.toml` with keys `user:<slug>`, written by `save_user_template` (an export). A marketplace is a further source, never a new format. `requirements.rs` names the program of each command step and lists what is missing from `PATH`; the editor shows that as warnings, never as errors.
