...
<div class="flex items-center space-x-2">
	...
	<div class="flex items-center gap-2">
		{#if isTransparent}
			<div
				class="border-muted-foreground/50 text-muted-foreground flex h-[30px] w-[30px] items-center justify-center rounded border border-dashed text-xs"
				title="Transparent"
			>
				∅
			</div>
		...
		{#if allowTransparent}
			<Button
				...
				onclick={isTransparent ? handleSetColor : handleClear}
			>
				{isTransparent ? 'Set Color' : 'Clear'}
			</Button>
		{/if}
	</div>
</div>
