...
mod tests {
    ...
    use crate::types::{
        HyprlandAnimationSettings, HyprlandDecorationSettings, HyprlandGeneralSettings,
        HyprlandInputSettings, LayoutMode,
    };
    ...
    #[test]
    fn input_snapshot_returns_defaults_when_no_overrides() {
        let (_temp_dir, service) = setup_service();
        let snapshot = service.load_input_snapshot().expect("load input snapshot");

        assert_eq!(snapshot.overrides, HyprlandInputSettings::default());
        let defaults = HyprlandInputSettings::with_defaults();
        assert_eq!(snapshot.effective, defaults);
    }
    ...
    #[test]
    fn persist_input_overrides_and_snapshot() {
        let (_temp_dir, service) = setup_service();

        let mut overrides = HyprlandInputSettings::default();
        overrides.kb_model = Some("pc105".into());
        overrides.kb_layout = Some("gb".into());
        overrides.kb_variant = Some("intl".into());
        overrides.kb_options = Some("caps:escape".into());
        overrides.touchpad.disable_while_typing = Some(false);
        overrides.touchpad.scroll_factor = Some(1.4_f32);
        overrides.touchpad.tap_button_map = Some("lrm".into());
        overrides.touchpad.middle_button_emulation = Some(true);

        service
            .persist_input_overrides(&overrides)
            .expect("persist input overrides");

        let contents = fs::read_to_string(service.override_path()).expect("read override file");
        assert!(contents.contains("\ninput {\n"));
        assert!(contents.contains("kb_model = pc105"));
        assert!(contents.contains("kb_layout = gb"));
        assert!(contents.contains("kb_variant = intl"));
        assert!(contents.contains("kb_options = caps:escape"));
        assert!(contents.contains("    touchpad {"));
        assert!(contents.contains("disable_while_typing = false"));
        assert!(contents.contains("scroll_factor = 1.4"));
        assert!(contents.contains("tap_button_map = lrm"));
        assert!(contents.contains("middle_button_emulation = true"));
        assert!(contents.contains("\ndecoration {\n"));
        assert!(contents.contains("\nanimations {\n"));

        let snapshot = service
            .load_input_snapshot()
            .expect("reload input snapshot");
        assert_eq!(snapshot.overrides.kb_model, Some("pc105".into()));
        assert_eq!(snapshot.overrides.kb_layout, Some("gb".into()));
        assert_eq!(snapshot.overrides.kb_variant, Some("intl".into()));
        assert_eq!(snapshot.overrides.kb_options, Some("caps:escape".into()));
        assert_eq!(
            snapshot.overrides.touchpad.disable_while_typing,
            Some(false)
        );
        assert_eq!(snapshot.overrides.touchpad.scroll_factor, Some(1.4_f32));
        assert_eq!(
            snapshot.overrides.touchpad.tap_button_map,
            Some("lrm".into())
        );
        assert_eq!(
            snapshot.overrides.touchpad.middle_button_emulation,
            Some(true)
        );
    }
    ...
    #[test]
    fn input_persist_skips_default_values() {
        let (_temp_dir, service) = setup_service();

        let mut overrides = HyprlandInputSettings::with_defaults();
        overrides.kb_model = Some("pc104".into());

        service
            .persist_input_overrides(&overrides)
            .expect("persist input overrides without defaults");

        let contents = fs::read_to_string(service.override_path()).expect("read override file");
        assert!(contents.contains("kb_model = pc104"));
        assert!(!contents.contains("kb_layout = us"));
        assert!(!contents.contains("kb_variant ="));
        assert!(!contents.contains("kb_options ="));
        assert!(!contents.contains("    touchpad {"));
    }

    #[test]
    fn loads_existing_touchpad_overrides() {
        let (_temp_dir, service) = setup_service();
        ...
        fs::write(
            service.override_path(),
            "# Generated by Omarchist - Hyprland overrides\n# Manual edits may be overwritten. Use the Omarchist UI.\n\ninput {\n    natural_scroll = true\n    touchpad {\n        scroll_factor = 1.7\n        tap_to_click = false\n    }\n}\n",
        )
        .expect("seed touchpad overrides");

        let snapshot = service
            .load_input_snapshot()
            .expect("load input snapshot with touchpad overrides");

        assert_eq!(snapshot.overrides.natural_scroll, Some(true));
        assert_eq!(snapshot.overrides.touchpad.scroll_factor, Some(1.7_f32));
        assert_eq!(snapshot.overrides.touchpad.tap_to_click, Some(false));
    }
