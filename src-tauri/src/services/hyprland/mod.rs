...
use crate::types::{
    general_field_registry, snap_field_registry, GeneralField, HyprlandConfigError,
    HyprlandGeneralOverrideMaps, HyprlandGeneralSettings, HyprlandGeneralSnapshot, HyprlandResult,
    HyprlandValue, SnapField,
};
...
impl HyprlandConfigService {
    ...
    pub fn persist_general_overrides(
        ...
        self.write_override_file(&override_maps, &parsed.preserved_lines)
    }
    ...
    fn parse_override_file(&self) -> HyprlandResult<ParsedOverrides> {
        ...
        for line in reader.lines() {
            ...
            if is_section_start(trimmed, "snap") {
            ...
            if in_snap_section {
                if let Some(field) = SnapField::from_key(&key) {
                    ...
                    warn!("Encountered unsupported Hyprland snap key '{key}', preserving original line");
                    preserved_lines.push(line);
                }
                continue;
            }
    ...
    fn write_override_file(
        &self,
        overrides: &HyprlandGeneralOverrideMaps,
        ...
        for field in general_field_registry() {
            if let Some(value) = overrides.general.get(field.key()) {
                wrote_entry = true;
                buffer.push_str("    ");
        ...
        if !overrides.snap.is_empty() {
            wrote_entry = true;
            buffer.push('\n');
            buffer.push_str("    snap {\n");
            for field in snap_field_registry() {
                if let Some(value) = overrides.snap.get(field.key()) {
                    buffer.push_str("        ");
                    buffer.push_str(field.key());
                    buffer.push_str(" = ");
                    buffer.push_str(&value.to_string());
