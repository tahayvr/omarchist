...
mod tests {
    ...
    fn test_validate_valid_theme() {
        let theme = json!({
            ...
            "modified_at": "2025-10-05T00:00:00Z",
            "apps": {
                "alacritty": {
                    "colors": {
                        "primary": {
        ...
        assert!(ThemeValidator::validate_theme(&theme).is_ok());
    }
    ...
    fn test_validate_missing_fields() {
        ...
        assert!(ThemeValidator::validate_theme(&theme).is_err());
    }
    ...
    fn test_validate_color() {
        assert!(ThemeValidator::validate_color("#fff"));
        assert!(ThemeValidator::validate_color("#ffffff"));
        assert!(ThemeValidator::validate_color("#ffffffff"));
        assert!(ThemeValidator::validate_color("255,255,255"));
        assert!(ThemeValidator::validate_color("255,255,255,1.0"));
        assert!(!ThemeValidator::validate_color("#gg"));
        assert!(!ThemeValidator::validate_color("256,0,0"));
    }

    #[test]
    fn test_is_omarchist_theme() {
        let valid_theme = json!({
            "name": "Test",
            "created_at": "2025-10-05T00:00:00Z",
            "modified_at": "2025-10-05T00:00:00Z",
            "apps": {}
        ...
        assert!(ThemeValidator::is_omarchist_theme(&valid_theme));

        let invalid_theme = json!({
            "title": "Some other theme",
            "colors": {}
        });

        assert!(!ThemeValidator::is_omarchist_theme(&invalid_theme));
    }
}
